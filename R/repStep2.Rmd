...
subtitle: "Steps 2 and 3, mesures anthropométriques et biologiques"
output:  
  html_document: 
    mode: selfcontained
    toc: yes 
...
```{r tab_2_1_5}
survey::svydesign(id = ~ uid,
                  weights = ~ w1, strata = ~gstratum + Age,
                  data = steps[enroll == 1 &
...
### Tableau 2.1.6 Prévalence du surpoids
...
survey::svydesign(id = ~ uid,
  ...
  tbl_svysummary(include = c(Surpoids),
                 statistic = list(all_categorical() ~ "{p}")) |>
  modify_header(label ~ "") |>
  add_ci(
    pattern = "{stat} ({ci})",
...
### Tableau 2.1.7 Prévalence de l'obésité
...
survey::svydesign(id = ~ uid,
  ...
  tbl_svysummary(include = c(Obese),
...
### Tableau 2.2.1 Prévalence de l'hypertension artérielle (HTA)
```{r tab_2_2_1}
survey::svydesign(id = ~ uid,
                  weights = ~ w2,
                  strata = ~gstratum + agegr,
...
### Tableau 2.3.1 Prévalence du diabète
```{r tab_2_3_1}
survey::svydesign(id = ~ uid,
                  weights = ~ w2,
                  strata = ~gstratum + agegr,
  ...
  tbl_svysummary(include = c(Diabete),
                 statistic = list(all_categorical() ~ "{p}")) |>
  modify_header(label ~ "") |>
  add_ci(
    pattern = "{stat} ({ci})",
