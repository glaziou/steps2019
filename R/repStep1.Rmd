...
### Tableau 1.1.1a Fumeurs de produits à base de tabac (T1)
```{r tab_1_1_1a}
d <- survey::svydesign(
  id = ~ uid,
  weights = ~ w1,
...
### Tableau 1.1.1b Fumeurs, par âge et sexe (T1)
```{r tab_1_1_1b}
...
### Tableau 1.1.2a Fumeurs quotidiens (T2)
```{r tab_1_1_2a}
d <- survey::svydesign(
  id = ~ uid,
  weights = ~ w1,
...
### Tableau 1.2.3b Fumeurs, par âge et sexe (X4)
```{r tab_1_2_3b}
...
### Tableau 1.3.8a Au cours des 30 derniers jours, plus grand nombre de verres d'alcool bus en une fois (A8)
...
### Tableau 1.3.8b Plus grand nombre de verres en une fois, par âge et sexe (A8)
```{r tab_1_3_8b}
...
### Tableau 1.3.9a Au cours des 30 derniers jours, nombre de fois au moins 6 verres d'alcool ont été bus en une occasion (A9)
...
### Tableau 1.3.9b Nombre de fois au moins 6 verres, par âge et sexe (A9)
```{r tab_1_3_9b}
...
### Tableau 1.3.10a Au cours des 7 derniers jours, nombre de verres d'alcool ont été bus par jour (A10)
...
### Tableau 1.3.10b Nombre de verres, par sexe (A10)
```{r tab_1_3_10b}
tbl_svysummary(
  d,
  include = c(Lundi, Mardi, Mercredi, Jeudi, Vendredi, Samedi, Dimanche),
...
### Tableau 1.3.10c Nombre de verres, par groupe d'âge (A10)
```{r tab_1_3_10c}
tbl_svysummary(
  d,
  include = c(Lundi, Mardi, Mercredi, Jeudi, Vendredi, Samedi, Dimanche),
...
### Tablean 1.3.11a Problèmes de famille à cause de l'alcool au cours des 12 derniers mois (A16)
```{r tab_1_3_11a}
d <- survey::svydesign(
  id = ~ uid,
  weights = ~ w1,
...
### Tableau 1.3.11b Fréquence, par sexe (A16)
```{r tab_1_3_11b}
tbl_svysummary(d,
               include = c(A16, sexe),
               label = list(A16 ~ "Fréquence des problèmes"),
...
### Tableau 1.3.11c Fréquence, par âge (A16)
```{r tab_1_3_11c}
tbl_svysummary(d,
               include = c(A16, agegr),
               label = list(A16 ~ "Fréquence des problèmes"),
...
### Tablean 1.4.7a Ajouter du sel ou sauce de soja en mangeant (avant ou pendant) (D5)
...
### Tableau 1.4.7b Fréquence, par sexe (D5)
```{r tab_1_4_7b}
tbl_svysummary(d,
               include = c(D5, sexe),
               missing = "no",
...
### Tablean 1.4.8a Ajouter du sel ou sauce de soja en faisant la cuisine (D6)
...
### Tableau 1.4.8b Fréquence, par sexe (D6)
```{r tab_1_4_8b}
tbl_svysummary(d,
               include = c(D6, sexe),
               missing = "no",
...
### Tablean 1.4.9a Manger des aliments riches en sel (D7)
...
### Tableau 1.4.9b Fréquence, par sexe (D7)
```{r tab_1_4_9b}
tbl_svysummary(d,
               include = c(D7, sexe),
               missing = "no",
...
### Tablean 1.4.10a Quantité de sel consommée (D8)
...
### Tableau 1.4.10b Quantité de sel, par sexe (D8)
```{r tab_1_4_10b}
tbl_svysummary(d,
               include = c(D8, sexe),
               missing = "no",
...
### Tableau 1.5.1 Attitudes alimentaires vis-à-vis du sel (D10-D11)
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1, .(
    ...
    sexe,
    D10 = qcm(diet.D10, 1),
    D11a = qcm(diet.salt.D11a, 1),
    D11b = qcm(diet.salt.D11b, 1),
    D11d = qcm(diet.salt.D11d, 1),
    D11e = qcm(diet.salt.D11e, 1)
  )]
)
...
tbl_svysummary(
  ...
  label = list(
    D10 ~ "Trop de sel est source de problèmes",
    D11a ~ "Il faut réduire les plats cuisinés salés",
    D11b ~ "Il faut vérifier la teneur en sel sur les étiquettes",
    D11d ~ "Il faut utiliser des épices autre que le sel",
    D11e ~ "Il faut éviter de manger hors du domicile"
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
...
tbl_svysummary(
  d,
  include = c(X11),
  label = list(X11 ~ "Nombre de portions"),
...
### Tableau 1.6.5a Activités de forte intensité au travail: nombre de jours par semaine (P5)
...
### Tableau 1.6.6a Jours d'activités de forte intensité au travail: nombre d'heures par jour (P6)
...
### Tableau 1.6.7a Trajets d'au mopins 10 minutes à pied ou à vélo (P7)
...
### Tableau 1.6.16a Comportement édentaire: nombre d'heures par jour (P16)
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1, .(
    ...
    bmigr,
    sexe,
    H1=yesno(hx_raised_bp.H1, T),
    H2a=yesno(hx_raised_bp.H2a, T),
    H2b=yesno(hx_raised_bp.H2b, T),
    H3=yesno(hx_raised_bp.H3, T),
    H4=yesno(hx_raised_bp.H4, T),
    H5=yesno(hx_raised_bp.H5, T)
  )]
)
...
tbl_svysummary(
  ...
  label = list(
    H1 ~ "TA déjà mesurée par un professionnel (H1) ",
    H2a ~ "Un professionnel vous a dit que vous aviez une TA élevée (H2a)",
    H2b ~ "Vous a t-on dit cela ces 12 derniers mois (H2b)",
    H3 ~ "Médicaments prescrits ces 2 dernières semaines pour HTA (H3)",
    H4 ~ "Avez vous vu un tahua pour votre HTA (H4)",
    H5 ~ "Prenez vous un remède traditionnel pour votre HTA (H5)"
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1, .(
    ...
    bmigr,
    sexe,
    H6=yesno(hx_diabetes.H6, T),
    H7a=yesno(hx_diabetes.H7a, T),
    H7b=yesno(hx_diabetes.H7b, T),
    H8=yesno(hx_diabetes.H8, T),
    H9=yesno(hx_diabetes.H9, T),
    H10=yesno(hx_diabetes.H10, T),
    H11=yesno(hx_diabetes.H11, T)
  )]
)
...
tbl_svysummary(
  ...
  label = list(
    H6 ~ "Glycémie déjà mesurée par un professionnel (H6) ",
    H7a ~ "Un professionnel vous a dit que vous aviez du diabète (H7a)",
    H7b ~ "Vous a t-on dit cela ces 12 derniers mois (H7b)",
    H8 ~ "Médicaments prescrits ces 2 dernières semaines pour le diabète (H8)",
    H9 ~ "Insuline prescrite pour le diabète (H9)",
    H10 ~ "Avez vous vu un tahua pour votre HTA (H10)",
    H11 ~ "Prenez vous un remède traditionnel pour votre HTA (H11)"
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1, .(
    ...
    bmigr,
    sexe,
    H12=yesno(hx_chol.H12, T),
    H13a=yesno(hx_chol.H13a, T),
    H13b=yesno(hx_chol.H13b, T),
    H14=yesno(hx_chol.H14, T),
    H15=yesno(hx_chol.H15, T),
    H16=yesno(hx_chol.H16, T)
  )]
)
...
tbl_svysummary(
  ...
  label = list(
    H12 ~ "Cholestérol déjà mesurée par un professionnel (H12) ",
    H13a ~ "Un professionnel vous a dit que vous aviez un cholestérol élevé (H13a)",
    H13b ~ "Vous a t-on dit cela ces 12 derniers mois (H13b)",
    H14 ~ "Médicaments prescrits ces 2 dernières semaines pour le cholestérol élevé (H14)",
    H15 ~ "Avez vous vu un tahua pour votre cholestérol élevé (H15)",
    H16 ~ "Prenez vous un remède traditionnel pour votre HTA (H16)"
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1, .(
    ...
    bmigr,
    sexe,
    H17=yesno(hx_cardiovascular.H17, T)
  )]
)
...
tbl_svysummary(
  ...
  label = list(
    H17 ~ "Avez-vous déjà eu une crise cardiaque ou angine de poitrine ou AVC (H17) "
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1, .(
    ...
    bmigr,
    sexe,
    H20=yesno(lifestyle_advice.H20, T),
    H20a=yesno(lifestyle_advice.lifestyle.H20a, T),
    H20b=yesno(lifestyle_advice.lifestyle.H20b, T),
    H20c=yesno(lifestyle_advice.lifestyle.H20c, T),
    H20d=yesno(lifestyle_advice.lifestyle.H20d, T),
    H20e=yesno(lifestyle_advice.lifestyle.H20e, T),
    H20f=yesno(lifestyle_advice.lifestyle.H20f, T),
    H20g=yesno(lifestyle_advice.lifestyle.H20g, T)
  )]
)
...
tbl_svysummary(
  ...
  label = list(
    H20 ~ "A consulté un professionnel au cours des 12 derniers mois (H20) ",
    H20a ~ "Reçu conseil d'arrêter de fumer (H20a)",
    H20b ~ "Reçu conseil de réduire la consommation de sel (H20b)",
    H20c ~ "Reçu conseil de manger au moins 5 portions de frtuis et légumes par jour (H20c)",
    H20d ~ "Reçu conseil de réduire la consommation de graisse (H20d)",
    H20e ~ "Reçu conseil de commencer ou faire plus d'activités physiques (H20e)",
    H20f ~ "Reçu conseil de maintenir un pods sain ou perdre du poids (H20f)",
    H20g ~ "Reçu conseil de réduire la consommation de boissons sucrées (H20g)"
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
...
d <- survey::svydesign(
  ...
  data = steps[enroll == 1 & sex == "F", .(
    ...
    bmigr,
    sexe,
    var=yesno(lifestyle_advice.cervical_cancer.CX1, T)
  )]
)
...
tbl_svysummary(
  ...
  include = var,
  label = list(
    var ~ "Déjà eu un dépistage du cancer du col utérin (CX1)"
  ),
  statistic = list(all_categorical() ~ "{p}"),
  missing = "no"
) |>
